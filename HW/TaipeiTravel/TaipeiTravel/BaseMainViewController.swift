...
class BaseMainViewController: UIViewController, CLLocationManagerDelegate, UITableViewDelegate, UITableViewDataSource, URLSessionDelegate, URLSessionDownloadDelegate {
    ...
    // User limit set default 10 km
    var UserLimit: Double! = 10000.0
    ...
    func addTable(_ filePath :URL?) {
        ...
        self.myTableView = UITableView(frame: CGRect(x: 0, y: 70, width: self.fullSize.width, height: self.fullSize.height - 200), style: .plain)
        // 設置委任對象
...
// MARK: functional methods
    ...
    func updateLimitDistance (userLocation: CLLocation){
        // 將所有資料依照與使用者距離重新排序
