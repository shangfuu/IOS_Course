...
class MainScene: SKScene, SKPhysicsContactDelegate {
    ...
    func createScene(){
        ...
        // create Score board
        let scoreBoard = SKLabelNode(text: "Score: \(MainScene.score)")
        
        scoreBoard.position = CGPoint(x: self.frame.minX + 100, y: self.frame.minY + 50)
        scoreBoard.fontName = "Avenir-Oblique"
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        .node?.name == "coins"{
            ...
            // Score board Add point
            MainScene.score += 100
            if let label = self.childNode(withName: "scoreBoard") as? SKLabelNode{
                label.text = "Score \(MainScene.score)"
            }
        }
    }
}
